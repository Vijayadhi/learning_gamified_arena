...
function AdminControls({ data, onChanged, error, setError }: { data: ClassroomState; onChanged: (value: ClassroomState) => void; error: string; setError: (value: string) => void }) {
  ...
  const importRows = async (event: ChangeEvent<HTMLInputElement>, action: "learners" | "bulk-content" | "bulk-questions") => {
    ...
    await save(action === "learners" ? { action, records } : { action, subjectId: selectedSubject, records }); event.target.value = "";
  };
  ...
  return <section className="admin-workspace">
    ...
    <div className="admin-grid">
      ...
      <article className="panel admin-card"><div className="panel-head"><div><p className="eyebrow">Step 3</p><h2>Learners & subjects</h2></div><UserPlus /></div><label className="file-import">Bulk upload learners (CSV/JSON)<input type="file" accept=".csv,.json,text/csv,application/json" onChange={(e) => importRows(e, "learners")} /></label><small>Required: <code>email</code>. Optional: <code>name</code>.</small><select value={learnerEmail} onChange={(e) => chooseLearner(e.target.value)}><option value="">Select learner to assign</option>{data.learners.map((item) => <option key={item.email} value={item.email}>{item.displayName || item.email} — {item.email}</option>)}</select><div className="assignment-checks">{allSubjects.map((item) => <label key={item.id}><input type="checkbox" checked={assignments.includes(item.id)} onChange={(e) => setAssignments((current) => e.target.checked ? [...current, item.id] : current.filter((id) => id !== item.id))} />{item.title}</label>)}</div><button onClick={() => save({ action: "assignment", email: learnerEmail, subjectIds: assignments })}>Save subject visibility</button><div className="admin-list">{data.learners.map((item) => <div key={item.email}><span>{item.displayName || item.email}<small>{item.email}</small></span><button onClick={() => save({ action: "learner-active", email: item.email, isActive: !item.isActive }, "PATCH")}><Power className={item.isActive ? "is-on" : ""} /></button></div>)}</div></article>
      <article className="panel admin-card"><div className="panel-head"><div><p className="eyebrow">Step 4</p><h2>Timed course access</h2></div><LockKeyhole /></div><select value={accessEmail} onChange={(e) => setAccessEmail(e.target.value)}><option value="">Learner</option>{data.learners.filter((item) => item.isActive).map((item) => <option key={item.email} value={item.email}>{item.email}</option>)}</select><select value={accessSubject} onChange={(e) => setAccessSubject(e.target.value)}><option value="">Subject</option>{allSubjects.map((item) => <option key={item.id} value={item.id}>{item.title}</option>)}</select><label>Starts<input type="datetime-local" value={from} onChange={(e) => setFrom(e.target.value)} /></label><label>Ends<input type="datetime-local" value={until} onChange={(e) => setUntil(e.target.value)} /></label><button onClick={() => save({ action: "access", email: accessEmail, subjectId: accessSubject, validFrom: from, validUntil: until })}>Create 4-digit code</button><div className="admin-list">{data.accessWindows.map((item) => <div key={item.id}><span><strong>{item.accessCode}</strong> · {item.email}<small>{allSubjects.find((s) => s.id === item.subjectId)?.title ?? item.subjectId} · until {new Date(item.validUntil).toLocaleString()}</small></span></div>)}</div></article>
